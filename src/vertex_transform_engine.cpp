...
VertexBank vertex_bank[2];

matrix44f V;
matrix44f P;    // VP is saved in register,
...
VertexSlot vertex_transform_core(int bank_id, VertexSlot input) {
  ...
  handling_a[1] *= inv_w;
  handling_a[2] *= inv_w;
  if (handling_a[0] < -1 || handling_a[0] > 1 ||
	  handling_a[1] < -1 || handling_a[1] > 1 ||
	  handling_a[2] < -1 || handling_a[2] > 1) {
	input.vertex_flag.master_matrix_id = 0xff;
	return input; // out of windows, delete
  }
  input.pos << handling_a[0], handling_a[1], handling_a[2], w; // XYZW
